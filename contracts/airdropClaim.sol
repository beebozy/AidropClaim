...
contract MerkleAirdrop is ERC20 {
    ...
    constructor(bytes32 _merkleRoot, IERC20 _token) ERC20("BEEB Token", "BBT") {
        owner = msg.sender; // Set the contract deployer as the owner
        merkleRoot = _merkleRoot; // Set the initial Merkle root
        token = _token; // Set the ERC20 token address

        _mint(address(this), 10000 * 10 ** decimals()); // Mint 10,000 tokens to the contract
    }
    ...
    function claimAirdrop(uint256 _amount, bytes32[] calldata _merkleProof) external {
        require(!verifiedClaimers[msg.sender], "Airdrop already claimed."); // Ensure the user hasn't claimed yet
        ...
        verifiedClaimers[msg.sender] = true;

        // Transfer the tokens to the claimant
        require(token.transfer(msg.sender, _amount), "Token transfer failed.");

        emit ClaimDetails(_amount, msg.sender); // Emit the claim event
    }
